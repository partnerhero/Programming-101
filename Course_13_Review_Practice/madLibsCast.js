...
You're a mad libs expert and decide to create your own, hilarious mad lib generating program. Essentially this program will have you cast actors into three different roles (parent, child, grandparent) for a tv show and then generate a random trio everytime you call it. You can pick anything or anyone you want to play these roles and the program will combine them for you as well as show you their total cost.

Steps:
1. The first thing we want to do is store our program in an object. Create an empty object named cast.
...
3. Create three properties inside the _roles object called parent, child, and grandparent. Each of these properties should be an empty array. These will be used to store the actors' names being pushed to each role.
...
const cast = {
    ...
    generateRandomTrio() {
        ...
        return `${parent.name} will be the parent, ${child.name} will be the child, and ${grandparent.name} will be the grandparent. This will cost you $${totalCost}`;
    },
};

cast.addActorToRole('parent', 'David Blaine', 150);
cast.addActorToRole('parent', 'A potato', 90);
cast.addActorToRole('parent', "Johnny Cash's corpse", 0.5);
cast.addActorToRole('parent', 'A cat named The Dog', 1000);
cast.addActorToRole('parent', 'Skrillex', 5000);

cast.addActorToRole('child', 'The sound my uncle makes when he sits down', 25);
cast.addActorToRole('child', 'A bottle of tears', 700);
cast.addActorToRole('child', 'Corey Feldman', 6);
cast.addActorToRole('child', "Shaq's left shoe", 100);
cast.addActorToRole('child', 'Wosh Jilson', 10);

cast.addActorToRole('grandparent', 'Rick Astley', 666);
cast.addActorToRole('grandparent', 'Wilson the volleyball', 100000);
cast.addActorToRole('grandparent', 'A pair a nice glasses', 3000);
cast.addActorToRole('grandparent', 'Seabiscuit', 2000);
cast.addActorToRole('grandparent', 'A guy who ALWAYS comes in late', 15);
